...
from dtos.proxy import ProxyDTO, PostProxyDTO, PutProxyDTO
...
async def get_all_proxies(
    ...
    proxy_values = ProxyManager(db, logger).get_all_actives(page_limit=page_limit, page_offset=page_offset)
...
async def get_one_proxy(proxy_id: int, db: AsyncSession = Depends(get_db)):
    proxy = ProxyManager(db, logger).get_by_id(proxy_id)
...
async def create_proxy(proxy: PostProxyDTO, db: AsyncSession = Depends(get_db)):
    proxy_value = ProxyManager(db, logger).post(
...
async def update_metric(proxy_id: int, proxy: PutProxyDTO, db: AsyncSession = Depends(get_db)):
    metric_value = ProxyManager(db, logger).edit_proxy(
        proxy_id,
        port=proxy.port,
